...
| `sign_wlasl100.py` | WLASL100 test clips through the sign cascade with and without the TGCN tier; `--select` first settles keypoint layout, coordinates and class order for the published weights on other clips; `--trained` scores the weights from `sign_train_tgcn.py` | 5.2, Table 5.1 |
| `sign_keypoints.py` | MediaPipe keypoints for the WLASL100 train, validation and test clips, extracted with the application's own adapter | 5.2 |
| `sign_train_tgcn.py` | TGCN trained on those keypoints; starting weights, coordinates, epoch and display threshold chosen on validation clips; test clips scored once | 5.2, Tables 3.1 and 5.1 |
...
- Datasets are not committed: ESC-50 is fetched by `scripts/download_esc50.py`;
  ...
  (see that folder's README); the WLASL100 clips come from the Voxel51 mirror
  on Hugging Face (`backend/data/WLASL-master/videos_trainval100/`, `videos_test100/`).
